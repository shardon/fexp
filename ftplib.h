...
#define FTPLIB_SDEBUG 6
#define FTPLIB_RDEBUG 7
#define FTPLIB_DEBUGARG 8
...
typedef void (*FtpDebug)(netbuf *nControl, const char *msg, void *arg);
...
GLOBALREF int FtpConnect(const char *host,FtpDebug sdebug,FtpDebug rdebug,void *darg,long timeout,netbuf **nControl);
...
GLOBALREF int FtpAccess(const char *path, int typ, int mode,off_t *startpoint, netbuf *nControl,
    netbuf **nData);
...
GLOBALREF int FtpSize(const char *path, off_t *size, char mode, netbuf *nControl);
